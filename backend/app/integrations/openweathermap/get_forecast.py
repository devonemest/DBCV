"""OpenWeatherMap Get Forecast интеграция используя httpx для прямых HTTP запросов."""
...
class OpenweathermapGetForecastIntegration(BaseIntegration):
    """Интеграция для получения прогноза погоды через OpenWeatherMap API."""
    
    @property
    def metadata(self) -> IntegrationMetadata:
        return IntegrationMetadata(
            id="openweathermap_get_forecast",
            version="1.0.0",
            name="OpenWeatherMap Get Forecast",
            description="Получение прогноза погоды на 5 дней через OpenWeatherMap API",
            category="weather",
            ...
            config_schema={
                ...
                "properties": {
                    ...
                    "cnt": {
                        ...
                        "minimum": 1,
                        "maximum": 40,
                        "description": "Количество прогнозов (максимум 40, по умолчанию 40)"
                    }
                }
            },
    ...
    async def execute(
        ...
        """
        Выполняет интеграцию используя httpx для прямых HTTP запросов к OpenWeatherMap API.
        ...
        params = {
            ...
            "units": units,
            "lang": lang,
            "cnt": min(max(1, cnt), 40)  # Ограничиваем от 1 до 40
        }
        
        # Проверяем, это координаты (lat,lon) или название города
        # Координаты должны быть в формате "lat,lon" где оба числа
        if "," in location:
            try:
